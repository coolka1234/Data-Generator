...
from fk_keys_hash_sets import (
    load_set,
    inspector_fk_keys_set,
    save_set,
    passenger_fk_keys_set,
)
...
def add_fine(fk_passanger, fk_inspector, amount, issue_date, status, deadline):
    insert_stmt_ed = fines_table.insert().values(
        fk_passenger=fk_passanger,
        fk_inspector=fk_inspector,
        amount=amount,
...
def generate_fines(how_many, num_of_passengers, num_of_ticket_inspectors):
    ...
    for _ in range(how_many):
        ...
        while f_key_inspector in inspector_fk_keys_set:
            f_key_inspector = fake.random_int(1, num_of_ticket_inspectors)
        issue_date = fake.date_time_between(
            date(year=2019, month=1, day=1), date(year=2023, month=12, day=31)
        )
