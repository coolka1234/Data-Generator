...
def add_vehicle(vehicle_number, last_service_date, production_date, capacity, type, status, air_conditioning):
...
def generate_purchases(how_many):
    ...
    for _ in range(how_many):
        ...
        while vehicle_number in vehicles_numbers_set:
            vehicle_number=fake.random_int(1000, 9999)
        add_vehicle(vehicle_number, fake.date_time_this_decade(0, 5), fake.date_time_this_decade(8, 15), fake.random_int(min=1, max=100), fake.random_element(elements=('bus', 'tram')), fake.random_element(elements=('active', 'inactive')), fake.random_element(elements=(True, False)))
