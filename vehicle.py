from connect_to_db import execute_query, engine, metadata
from sqlalchemy import Table, insert
from faker import Faker
import sys
fake = Faker('pl_PL')
vehicles_table = Table('vehicles', metadata, autoload_with=engine)

def add_vehicle(date, amount):
    insert_stmt_vehicle = vehicles_table.insert().values(date=date, amount=amount)
    execute_query(insert_stmt_vehicle)

def generate_purchases(how_many):
    for _ in range(how_many):
        add_vehicle(fake.date_time_this_year(), abs(fake.pyfloat(2, 2)))

if __name__ == '__main__':
    generate_purchases(int(sys.argv[1]))

